...
def parse_metadata(mem_file):
    """Read name, degree, and initialisation from the top of ID_x.txt"""
    name, degree, initialisation = None, None, False
    if mem_file.exists():
        with open(mem_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                elif line.lower().startswith("initialisation:"):
                    initialisation = line.split(":", 1)[1].strip().lower() == "true"
                if name and degree and initialisation is not None:
                    break
    return name, degree, initialisation
...
def append_to_memory(fid, user_text, winnie_text):
    ...
    name, degree, initialisation = parse_metadata(mem_file)
    with open(mem_file, "w", encoding="utf-8") as f:
        f.write(f"name: {name or ''}\n")
        f.write(f"degree: {degree or ''}\n")
        f.write(f"initialisation: {initialisation}\n\n")
        f.write(f"--- Conversation Log ---\n")
        f.write(get_conversation(mem_file))
...
def set_initialisation(fid, value):
    ...
    mem_file = get_memory_file(fid)
    if mem_file.exists():
        name, degree, _ = parse_metadata(mem_file)
        conv = get_conversation(mem_file)
        with open(mem_file, "w", encoding="utf-8") as f:
            f.write(f"name: {name or ''}\n")
            f.write(f"degree: {degree or ''}\n")
...
if __name__ == "__main__":
    try:
        while True:
            ...
            name, degree, initialisation = parse_metadata(memory_file)
            print(f"Talking to {name or 'Unknown'} ({degree or 'Unknown degree'}, Initialised: {initialisation})")
